...
class _CustomCheckboxState extends State<CustomCheckbox> with TickerProviderStateMixin, ToggleableStateMixin {
  late _CheckboxPainter _painter;
  bool? _previousValue;

  @override
  void initState() {
    _painter = _CheckboxPainter(edgeSize: widget.width, strokeWidth: widget.strokeWidth);
    super.initState();
  ...
  Widget build(BuildContext context) {
    ...
    final Color effectiveActiveColor = widget.fillColor?.resolve(activeStates) ?? _widgetFillColor.resolve(activeStates) ?? themeData.checkboxTheme.fillColor?.resolve(activeStates) ?? _defaultFillColor.resolve(activeStates);
    final Color effectiveInactiveColor = widget.fillColor?.resolve(inactiveStates) ?? _widgetFillColor.resolve(inactiveStates) ?? themeData.checkboxTheme.fillColor?.resolve(inactiveStates) ?? _defaultFillColor.resolve(inactiveStates);
    ...
    final Color effectiveActivePressedOverlayColor = widget.overlayColor?.resolve(activePressedStates) ?? themeData.checkboxTheme.overlayColor?.resolve(activePressedStates) ?? effectiveActiveColor.withAlpha(kRadialReactionAlpha);

    final Set<MaterialState> inactivePressedStates = inactiveStates..add(MaterialState.pressed);
    final Color effectiveInactivePressedOverlayColor = widget.overlayColor?.resolve(inactivePressedStates) ?? themeData.checkboxTheme.overlayColor?.resolve(inactivePressedStates) ?? effectiveActiveColor.withAlpha(kRadialReactionAlpha);
...
// const double _kEdgeSize = widget.width;
...
class _CheckboxPainter extends ToggleablePainter {
  ...
  _CheckboxPainter({required this.edgeSize, required this.strokeWidth});
  ...
  Rect _outerRectAt(Offset origin, double t) {
    ...
    final double size = edgeSize - inset * strokeWidth;
  ...
  void _drawCheck(Canvas canvas, Offset origin, double t, Paint paint) {
    ...
    Offset start = Offset(edgeSize * 0.25, edgeSize * 0.5);
    Offset mid = Offset(edgeSize * 0.45, edgeSize * 0.7);
    Offset end = Offset(edgeSize * 0.75, edgeSize * 0.35);
    if (t < 0.5) {
  ...
  void _drawDash(Canvas canvas, Offset origin, double t, Paint paint) {
    ...
    Offset end = Offset(edgeSize * 0.75, edgeSize * 0.5);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (previousValue == false || value == false) {
      ...
      if (t <= 0.5) {
        final BorderSide border = side ?? BorderSide(width: strokeWidth, color: paint.color);
